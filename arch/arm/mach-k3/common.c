...
void __noreturn jump_to_image_no_args(struct spl_image_info *spl_image)
{
	...
	if (board_is_resuming()) {
		...
		u64 tfa_address = *(ulong *)(LPM_BL31_START);
		...
		if (ret)
			/* TODO: panic */
			log_warning("%s: ATF failed to decrytp TFA\n", __func__);

		/* TODO: remove this */
		memcpy((void *)(ulong)tfa_address,
		       (void *)LPM_BL31, *(ulong *)(LPM_BL31_SIZE));

		ret = rproc_load(1, tfa_address, 0x200);
		if (ret)
			panic("%s: ATF failed to load on rproc (%d)\n", __func__, ret);
		/* TODO: and this ? no need to authenticate any more right ? */
